import { Search, Cable, Cog, BrainCircuit, TrendingUp, ChevronRight } from "lucide-react";
import SectionHeading from "@/src/components/Common/SectionHeading/SectionHeading";

const STEPS = [
  { icon: Search, title: "Discover", description: "Understand the factory and business requirements." },
  { icon: Cable, title: "Connect", description: "Connect machines, systems, and data sources." },
  { icon: Cog, title: "Automate", description: "Automate repetitive and critical workflows." },
  { icon: BrainCircuit, title: "Intelligence", description: "Apply AI and analytics." },
  { icon: TrendingUp, title: "Optimize", description: "Continuously improve performance." },
];
        ...
        <div className="mt-14 flex flex-col lg:flex-row items-stretch gap-3">
          {STEPS.map((step, i) => (
            <div key={step.title} className="flex flex-col lg:flex-row items-center gap-3 flex-1">
              ...
              {i < STEPS.length - 1 && (
                <ChevronRight size={20} className="shrink-0 rotate-90 lg:rotate-0 text-brand-300" />
              )}
            </div>
          ))}
        </div>
